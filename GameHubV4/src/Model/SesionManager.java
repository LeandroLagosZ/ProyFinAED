...
public class SesionManager {
    ...
    private final String FAVORITOS_FILE = "favoritos.txt";

    // Guarda la sesión actual escribiendo en los archivos .txt locales
    public void guardarSesion(Object[] historial, Object[] descargas, Object[] favoritos) {
        try {
            guardarArchivo(HISTORIAL_FILE, historial);
            guardarArchivo(DESCARGAS_FILE, descargas);
            guardarArchivo(FAVORITOS_FILE, favoritos);
    ...
    // Carga los datos de la lista de favoritos desde favoritos.txt
    public Videojuego[] cargarFavoritos() {
        return cargarArchivo(FAVORITOS_FILE);
    }
